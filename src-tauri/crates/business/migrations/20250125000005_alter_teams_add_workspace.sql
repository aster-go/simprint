...
ALTER TABLE teams ADD COLUMN workspace_uuid TEXT REFERENCES workspaces(uuid) ON DELETE CASCADE;
...
ALTER TABLE teams DROP COLUMN max_environments;
ALTER TABLE teams DROP COLUMN max_proxies;

-- 移除默认代理字段（不再需要默认代理）
ALTER TABLE teams DROP COLUMN default_proxy_uuid;
