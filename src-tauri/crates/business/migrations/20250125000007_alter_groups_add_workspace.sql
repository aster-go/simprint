...
ALTER TABLE groups ADD COLUMN workspace_uuid TEXT REFERENCES workspaces(uuid) ON DELETE CASCADE;
...
ALTER TABLE groups DROP COLUMN user_uuid;

-- 移除默认代理字段（不再需要默认代理）
ALTER TABLE groups DROP COLUMN default_proxy_uuid;
