...
ALTER TABLE proxies ADD COLUMN workspace_uuid TEXT REFERENCES workspaces(uuid) ON DELETE CASCADE;

-- 添加 owner_uuid 列（先允许 NULL，数据迁移后再设置为 NOT NULL）
ALTER TABLE proxies ADD COLUMN owner_uuid TEXT REFERENCES users(uuid) ON DELETE CASCADE;
...
ALTER TABLE proxies DROP COLUMN team_uuid;

-- 移除 usage_count（可通过查询计算）
ALTER TABLE proxies DROP COLUMN usage_count;
