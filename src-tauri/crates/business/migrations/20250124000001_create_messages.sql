...
CREATE TABLE IF NOT EXISTS messages (
    ...
    -- 发送者
    sender_uuid TEXT, -- 发送者 TEXT（系统消息可为 NULL）
    ...
    related_uuid TEXT, -- 关联资源的 TEXT（如 team_uuid, invitation_uuid）
...
CREATE TRIGGER trigger_auto_create_team_message_recipients
    ...
    FOR EACH ROW
    WHEN NEW.recipient_type = 'team'
      AND NEW.related_type = 'team'
      AND NEW.related_uuid IS NOT NULL
BEGIN
    -- 如果接收者类型是 team，且有关联的团队 TEXT
        -- 为团队所有活跃成员创建消息关联记录
